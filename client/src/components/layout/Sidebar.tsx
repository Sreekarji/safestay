...
import { LayoutDashboard, Map, FileText, ClipboardList, Settings, LogOut, Shield } from 'lucide-react';
...
const sidebarItems = [
  ...
  { to: '/report/new', icon: FileText, labelKey: 'nav.report', roles: ['student'] as const },
  { to: '/my-reports', icon: ClipboardList, labelKey: 'nav.myReports' },
  { to: '/admin', icon: Shield, labelKey: 'nav.admin', roles: ['admin'] as const },
  ...
  return (
    <aside className="hidden lg:flex lg:flex-col lg:w-64 fixed left-0 top-16 bottom-0 bg-white border-r border-slate-200">
      <nav className="flex-1 p-4 space-y-1">
        {sidebarItems
          .filter((item) => !item.roles || item.roles.includes(user?.role as never))
          .map((item) => (
