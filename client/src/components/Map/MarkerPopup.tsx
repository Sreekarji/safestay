  ...
  return (
    <div style={{ padding: '4px', fontFamily: 'Inter, sans-serif' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '8px' }}>
        <div>
          <h3 style={{ fontSize: '14px', fontWeight: 700, color: isDark ? '#f1f5f9' : '#1e293b', margin: 0, lineHeight: 1.3 }}>
            {marker.name}
          </h3>
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '6px' }}>
          <span style={{ fontSize: '11px', color: isDark ? '#94a3b8' : '#64748b', fontWeight: 500 }}>Safety Index</span>
        ...
        <div style={{ display: 'flex', alignItems: 'flex-end', gap: '8px' }}>
          ...
          <span style={{ fontSize: '12px', color: isDark ? '#64748b' : '#94a3b8', marginBottom: '4px' }}>/ 100</span>
        ...
        {/* Progress bar */}
        <div style={{ marginTop: '8px', height: '6px', background: isDark ? '#334155' : '#e2e8f0', borderRadius: '3px', overflow: 'hidden' }}>
      ...
      {monthIdx > 0 && (
        <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '8px', paddingLeft: '4px' }}>
          <span style={{ fontSize: '11px', color: isDark ? '#64748b' : '#94a3b8' }}>vs prev month:</span>
          <span style={{ fontSize: '11px', fontWeight: 700, color: diff >= 0 ? '#22c55e' : '#ef4444' }}>
            {diff >= 0 ? '+' : ''}{diff} pts
          </span>
          <span style={{ fontSize: '10px', color: isDark ? '#475569' : '#cbd5e1' }}>({prevScore} → {currentScore})</span>
        </div>
      )}

      {/* Stats row */}
      <div style={{ display: 'flex', alignItems: 'center', gap: '12px', fontSize: '11px', color: isDark ? '#94a3b8' : '#64748b' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '4px' }}>
          ...
          {currentScore >= 80 ? 'Verified' : 'Under review'}
        </div>
      </div>
    </div>
  );
