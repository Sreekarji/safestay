import type { MapMarkerWithHistory, SSIMonthly, AISummary, DashboardStats, SSITrend, AreaRisk, CategoryBreakdown, RecentReport } from '../types';
import { TIMELINE_MONTHS } from '../types';
import api from './api';

// ── Historical SSI Data Generator ────────────────────────────
function generateHistory(startScore: number, trend: 'up' | 'down' | 'stable' | 'volatile'): SSIMonthly[] {
  ...
  try {
    ...
    if (Array.isArray(result) && result.length > 0) {
      return result.map((m: any) => ({
        ...
        history: m.ssiHistory?.map((h: any) => ({ month: h.date?.slice(0, 7) || '2025-01', score: h.score })) ?? generateHistory(m.ssi ?? 50, 'stable'),
      }));
    }
    return SEED_MARKERS;
  } catch {
...
export async function fetchDashboardStats(): Promise<DashboardStats> {
  try {
    ...
    const avg = SEED_MARKERS.reduce((s, m) => s + m.ssi, 0) / SEED_MARKERS.length;
    return {
      ...
      highRiskCount: SEED_MARKERS.filter((m) => m.ssi < 40).length,
      mediumRiskCount: SEED_MARKERS.filter((m) => m.ssi >= 40 && m.ssi < 70).length,
      lowRiskCount: SEED_MARKERS.filter((m) => m.ssi >= 70).length,
    };
  }
}
...
export async function fetchAreaRisks(): Promise<AreaRisk[]> {
  try {
    ...
    return [
      ...
      { area: 'Kondapur', averageSSI: 58, reportCount: 12, accommodations: 15 },
      { area: 'Gachibowli', averageSSI: 68, reportCount: 15, accommodations: 18 },
